...
import AstroNeuDL.segmentation.visualizeRCNN as visualize
import matplotlib
import logging
# Agg backend runs without a display
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import AstroNeuDL.segmentation.utilsRCNN as utils
from AstroNeuDL.data_generator.data_generator import import_image
...
class NucleusDataset(utils.Dataset):
    ...
    def load_dataset(self, dataset_dir, subset, VAL_IMAGE_IDS):
        ...
        logging.info("Datasetdir {}".format(dataset_dir))
        if subset == "val":
        ...
        for image_id in image_ids:
            self.add_image(
                "image",
                image_id=image_id,
                path=os.path.join(dataset_dir, image_id, "image/{}.png".format(image_id))) ## HERE .png
...
def detect(model, dataset_dir, subset, RESULTS_DIR, VAL_IMAGE_IDS):
    ...
    for image_id in dataset.image_ids:
        logging.info("Test image id {}".format(image_id))
        ...
        logging.info("detection masks and scores: {} {}".format(np.shape(r["masks"]), np.shape(r["scores"])))
