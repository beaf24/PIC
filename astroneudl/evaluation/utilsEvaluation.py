...
def import_images(import_dir, files, new_file_ending):
	...
	if np.shape(data)[-1] == 1:
		data = data[...,0]
	logging.info("Shape of data imported is: {}".format(np.shape(data)))
	return data, names
...
def calcerrormap(prediction, groundtruth):
	...
	logging.info("The relative Error over the normalized dataset is: {} best ist close to zero".format(rel_error_norm))
...
def prepare_data_for_evaluation(root_dir, max_images):
	...
	if RCNN == True:
		...
		groundtruth = []
		predictions = []
		logging.info("importing {} files".format(len(image_fnames)))
		...
		logging.info("pred %s".format(np.shape(predictions)))
		logging.info("gt %s".format(np.shape(groundtruth)))
